...
如果 PDF 是书本、专著、教材、报告式长章节或学位论文，不要按期刊论文的摘要-方法-结果粒度处理。你必须先识别目录、章节标题、小节标题、表格清单、图件清单和页码范围，判断哪些章节与近海油气田多介质污染物决策平台有关，再按章节/主题展开提取。
...
当 `document_type` 判断为 `T4_thesis_book_chapter`，且不是 `R4_minimal_keep` 时，必须执行更高密度的章节化提取。书本、专著、教材、长报告和学位论文通常包含多个独立主题，不能只抽摘要、前言、结论或少数显眼表格。

**先做章节地图：**
- 在 `processing_notes` 中记录你实际覆盖的主要章节或页码范围。
- 如果有目录，优先用目录确定章节结构；如果没有目录，就用 PDF 中的一级/二级标题构建章节结构。
- 对每个相关章节判断它支持的决策任务，例如污染物识别、风险评估、监测、治理、运行、成本、法规、优化模型。

**书本需要优先多提这些内容：**
- 概念、分类、污染物谱系、来源类型、介质类型、迁移路径和暴露路径。
- 浓度范围、排放强度、组成比例、分布特征、阈值、限值、毒性终点、风险等级。
- 监测采样方法、前处理方法、检测技术、检出限、适用介质、质量控制要求。
- 治理/处理技术的工艺流程、适用条件、去除率、运行参数、优缺点、适用规模。
- 工程约束、平台空间/重量/能耗/维护约束、药剂/材料消耗、成本和经济性。
- 法规标准、管理要求、排放限值、环境容量、规划或管理建议。
- 章节中的典型案例、区域经验、对比表、流程图、模型框架和决策规则。

**书本的输出密度要求：**
- 对 R1/R2 的书本或长章节，除非文本质量很差，通常应输出 `15-40` 条 `knowledge_items`；如果有大量相关表格或案例，可以更多。
- 对 R3 的书本或长章节，通常应输出 `8-25` 条可迁移知识。
- `vector_index_records` 不限于普通论文的 2-5 条；书本应按主题生成 `6-12` 条，至少覆盖主要相关章节或主要决策任务。
- 如果因为 PDF 只给了节选、OCR 很差、页码缺失或内容低相关而无法达到上述密度，必须在 `quality_control.manual_review_recommendations` 或 `processing_notes` 中说明原因。

**书本知识条目的粒度：**
- 不要把整章压缩成一条泛泛结论。一个污染物类别、一个参数范围、一种技术、一条法规限值、一个监测方法、一个案例结论，原则上分别作为独立 `knowledge_item`。
- 表格中的多行有效数据应拆成多条或按同类项合并为带范围的条目；保留表号、页码和条件。
- 可以提取非数值型知识，但必须是对平台有用的规则、分类、流程、适用条件、限制或管理建议，不能只是科普性背景。
- 在 `context` 中尽量写入 `chapter`、`section`、`medium`、`pollutant_group`、`geographic_scope`、`source_type`、`applicability` 等字段，帮助后续入库检索。
...
如果是相关书本、专著、章节或学位论文，应改为按章节/主题生成 6-12 条向量库条目。每条对应一个独立检索意图，例如"采出水污染物组成与浓度范围"、"海上平台排放管理要求"、"含油废水处理工艺对比"、"海洋生态风险评价指标"。不要只生成题录型摘要。
...
- 对 `T4_thesis_book_chapter`，如果存在多个相关章节，应按相关章节提取，而不是以整本书主题过宽为理由最小保留。
...
如果是书本、专著、教材或书籍章节，尽量在 `bibliographic_metadata` 中额外补充：`book_title`、`chapter_title`、`editors`、`publisher`、`isbn`、`chapter_or_page_range`、`total_pages`、`edition`。未知时不要编造，可以省略或填 null。
